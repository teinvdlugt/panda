...
void panda::implementation::adjacencyDecomposition(int argc, char **argv,
   ...
   // Retrieve vertices
   const Matrix<Integer> &vertices_nsco1_coords = {known_output.begin(), known_output.begin() + 56};
   const Matrix<Integer> &vertices_nss_coords = {known_output.begin() + 56, known_output.end()};
   ...
   for (const auto &vertex: vertices_nss_coords) {
      ...
      const Row<Integer>& vertex_in_nsco1_coords = {vertices_nsco1_coords.at(j - 1).begin(), vertices_nsco1_coords.at(j - 1).end() - 6};
